...
def get_args():
    ...
    parser.add_argument('--exp', type=str, default=r'./Experiments/mle.json')
    parser.add_argument('--resume', type=str, default='./Experiments/checkpoints/epoch19_step75660.pth')
    args = parser.parse_args()
    return args

def get_data_loaders(opts, DataProvider): ## opts = self.opts['dataset']; DataProvider=cityscapes.DataProvider
...
class Trainer(object):
    def __init__(self, args): ##  self = Trainer()
        self.global_step = 0
        self.epoch = 0
    ...
    def save_checkpoint(self, epoch):
        ...
        save_name = os.path.join('model', 'epoch%d_step%d.pth'\
...
#                self.save_checkpoint(epoch)             
            ...
            if step % self.opts['print_freq'] == 0:
                ...
                self.writer.add_image('pred_mask', masks[0], self.global_step, dataformats='HWC')
                self.writer.add_image('gt_mask', masks[1], self.global_step, dataformats='HWC')
                self.writer.add_image('image', img, self.global_step, dataformats='HWC')
                self.writer.add_image('vertex_logits', vert_logits, self.global_step, dataformats='HWC')
                self.writer.add_image('edge_logits', edge_logits, self.global_step, dataformats='HWC')
                self.writer.add_image('edge_mask', edge_mask, self.global_step, dataformats='HWC')
                self.writer.add_image('vertex_mask', vertex_mask, self.global_step, dataformats='HWC')
                ...
                if self.opts['return_attention'] is True:
                    ...
                    self.writer.add_image('attention_1', pyramid_expand(_scale(att[0]), upscale=8, sigma=10), self.global_step, dataformats='HWC')
                    self.writer.add_image('attention_2', pyramid_expand(_scale(att[1]), upscale=8, sigma=10), self.global_step, dataformats='HWC')
                    self.writer.add_image('attention_3', pyramid_expand(_scale(att[2]), upscale=8, sigma=10), self.global_step, dataformats='HWC')
                ...
                print("[%s] Epoch: %d, Step: %d, Polygon Loss: %f, RCF-Edge Loss: %f, Vertex Loss: %f, Accuracy: %f, IOU: %f"\
                %(str(datetime.now().strftime('%Y-%m-%d %H:%M:%S')), epoch, self.global_step, accum['loss'], accum['fp_edge_loss'], accum['fp_vertex_loss'],\
