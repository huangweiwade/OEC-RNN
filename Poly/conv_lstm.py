...
class AttConvLSTM(nn.Module):
    def __init__(self, opts, feats_channels, feats_dim, n_layers=2,
        ...
        for l in range(self.n_layers):
            hidden_dim = self.hidden_dim[l]  #[64,16]
            if l != 0:
                in_channels = self.hidden_dim[l-1]      # [64]
            else:
                in_channels = self.feats_channels + 3   # [131]
...
if __name__ == '__main__':
    opts={}
    opts['correction_threshold'] = 0.5
    feats_channels = 128
    feats_dim = 28
    model = AttConvLSTM(opts,feats_channels,feats_dim)
    # print [c for c in model.children()]

    input_shape=[2,128,28,28]
