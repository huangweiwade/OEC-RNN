...
def process_info(args):
    ...
    """
    fname, opts = args #fname = json文件的路径
...
class DataProvider(Dataset):
    ...
    def read_dataset(self):
        # data_dir = "F:/polyrnn-pp-pytorch/cityscapes/val"
        data_list = glob.glob(osp.join(self.data_dir, '*.json'))   #路径可修改
        data_list = [[d, self.opts] for d in data_list]

        pool = multiprocessing.Pool(self.opts['num_workers'])
        data = pool.map(process_info, data_list)  # instance['component']
        pool.close()
        pool.join()
    ...
    def extract_crop(self, component, instance, context_expansion):
        ...
        new_img = transform.rescale(new_img, scale_factor, order=1, 
            preserve_range=True) #去掉了multichannel=True
